...
// Nejdříve jen vyzkoušíme jestli nám funguje CsvReader
        ...
        OnInvalidDataHandler errorHandler = (string row, int index) => { Console.WriteLine("Chybný řádek:" + row); };
        ...
        {
            Console.WriteLine(
                $"{team.ShortName} - {team.TeamName} | {team.WinCount} | {team.LossCount} | {team.TieCount}");
        }
        ...
        {
            Console.WriteLine(
                $"{match.HomeTeam.TeamName} vs {match.AwayTeam.TeamName} => {match.HomeGoals} : {match.AwayGoals}");
        }
